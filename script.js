...
var tools = {
    ...
    signature: function() {
        ...
        container.innerHTML = "<a href='profile.php?action=config&w=tools'><br>nCore Tools v" + GM_info.script.version + " by Kecs</a>";
    ...
    addMenuLink: function() {
        var list = document.getElementsByClassName("wiki_ul");
        list = list[0];
    ...
    showAdmin: function() {
        var userbox = document.getElementsByClassName("userbox_all");
        userbox = userbox[0];
        if (userbox.parentNode.name == "profil_beallitasa") {
            ...
            var header = document.getElementsByClassName("userbox_fej");
            header = header[0];
            header.innerHTML = text.header;
            var userbox = document.getElementsByClassName("userbox_tartalom");
            userbox = userbox[0];
    ...
    prepareValue: function(id, label, comment) {
        ...
        if (id == "searchPattern") {
            ...
            var showNo = "";
            if (parseInt(value)) {
                showYes = "selected='selected'";
            } else {
                showNo = "selected='selected'";
            }
...
var ncore = {
    removeAdsMain: function() {
        ...
        if (parseInt(value)) {
            var ads = document.getElementsByClassName("news_block_right");
            if (ads.length > 0) {
    ...
    removeAdsTorrent: function() {
        var value = GM_getValue("removeAds");
        if (parseInt(value)) {
            var content = document.getElementsByClassName("banner");
            if (content != null && content != undefined && content.length > 0) {
                for (var i = 0; i < content.length; i++) {
                    content[i].style.display = "none";
                }
            }
            var plus50p = document.getElementById("plus50pont");
            if (plus50p != null && plus50p != undefined) {
                plus50p.style.display = "none";
            }
            var plus50p = document.getElementById("upperad");
            if (plus50p != null && plus50p != undefined) {
                plus50p.style.display = "none";
            }
            var plus50p = document.getElementById("bottomad");
            if (plus50p != null && plus50p != undefined) {
                plus50p.style.display = "none";
            }
            var plus50p = document.getElementById("detailsad");
            if (plus50p != null && plus50p != undefined) {
                plus50p.style.display = "none";
            }
        }
    ...
    torrentIterator: function() {
        ...
        if (content != null && content != undefined && content.length > 0) {
            ...
            var searchPattern = GM_getValue("searchPattern");
            var patterns = new Array();
            if (searchPattern != null && searchPattern != undefined && searchPattern != "") {
                try {
                    var p = searchPattern.split("\n");
                    for (var i = 0; i < p.length; i++) {
                        patterns.push(eval("/" + p[i] + "/gi"));
                    }
                } catch (e) {
                    if (e.name == "SyntaxError" || e.name == "ReferenceError") {
                        var msg = text.searchPatternError.replace("[PATTERN]", p[patterns.length]);
                        msg = msg.replace("[ERROR]", e.message);
                        alert(msg);
                    } else {
                        throw e;
                    }
            ...
            for (var i = 0; i < torrents.length; i++) {
                ...
                if (box == null || box == undefined || box.length == 0) {
                    box = torrents[i].getElementsByClassName("torrent_txt2");
                }
        ...
        if (otherimages.length > 0) {
            for (var i = 0; i < otherimages.length; i++) {
                if (otherimages[i].parentNode.title == "Csatolt kép megnyitása") {
                    ...
                    image.setAttribute("style", "border:0px;max-width:600px;max-height:400px;");
                    image.src = imageURL;
